...
public class DimensionPractice {
	...
	public static void practice9() {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 2; j++) {
				if (findName.equals(group1[i][j])) {
					...
					System.out.printf("검색하신 %s 학생은 1분단 %c 번째 줄 %s에 있습니다.%n", findName, order[i], position);
					found = true;
				}
			}
		}
		...
		for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 2; j++) {
                if (group2[i][j].equals(findName)) {
                    String position = (j == 0) ? "왼쪽" : "오른쪽";
                    System.out.printf("검색하신 %s 학생은 2분단 %c 번째 줄 %s에 있습니다.%n", findName, order[i], position);
                    found = true;
                }
            }
        }
